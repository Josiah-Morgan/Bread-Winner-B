...
from utils.config.config import Colors, tips_and_tricks
...
async def get_tips(section: str, emoji: bool):
    if section == "all":
        tip = random.choice(list(tips_and_tricks.values()))
        tip = random.choice(tip)
    else:
        tip = random.choice(tips_and_tricks[section])

    if emoji == True:
        tip = "<:Info:1071919537357869098> " + f"`{tip}`"

    show_tip = random.randint(1, 8)
    if show_tip == 1:
        return tip
    else:
        return " "
...
async def auto_embed(title: str, description: str, footer_tip: bool, color=None):
    embed = disnake.Embed(
        title=title,
        description=f"{description}",
        color=color or disnake.Embed.get_default_color(),
    )

    if footer_tip == True:
        embed.set_footer(text=await get_tips("fonts"))
