...
class SnipeCommands(commands.Cog):
  ...
  async def on_message_delete(self, message: disnake.Message):
    ...
    for member in message.mentions:
      ...
      # No data means on
      if not gp_data and not message.author.bot:
  ...
  async def editsnipe(
    ...
    # Text
    bf_content = snipe["bf_content"]
    af_content = snipe["af_content"]
    ...
    embed = Embed(
      title=f"Message Sniped - {channel.name}", color=inter.author.color
    )
    embed.set_author(name=snipe["author"], icon_url=snipe["avatar_url"])
    embed.add_field(name="Before", value=bf_content)
    embed.add_field(name="After", value=af_content)
    ...
    if af_attachment:
      embed.add_field(
        name="After Attachment", value=f"||[{af_file_name}]({af_attachment})||"
      )
