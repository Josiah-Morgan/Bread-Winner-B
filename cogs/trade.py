...
from utils.signing_tools import team_check, send_notfication_channel, get_team_owner, roster_cap
...
async def get_trade_channel(inter):
  ...
  if trade_channel_id:
    trade_channel = inter.guild.get_channel_or_thread(int(trade_channel_id[0]))
    if trade_channel:
      return trade_channel

  sign_channel_id = await Database.get_data('SigningChannel', inter.guild.id)
  if sign_channel_id:
    sign_channel = inter.guild.get_channel_or_thread(int(sign_channel_id[0]))
    if sign_channel:
      return sign_channel
...
class TradeCommands(commands.Cog):
  ...
  async def trade_block(self, inter, player: disnake.Member, team: disnake.Role = None):
    ...
    if not team_role:
      return await inter.send(f"You don't have [team roles saved]({Links.premium_link}), so please use the `your_team` parameter", ephemeral=True)
  ...
  async def trade(self, inter, team: disnake.Role, your_team: disnake.Role = None):
    ...
    if not author_team_role:
      return await inter.send(f"You don't have [team roles saved]({Links.premium_link}), so please use the `your_team` parameter")
