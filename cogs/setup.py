...
#async def add_setting_once(self, inter, database):
    #response = await get_user_response_numbers(self, inter)
    #if not response:
        #return await inter.send("No information was sent", ephemeral=True)

    #await Database.add_data(database, inter.guild.id, response)
    #embed = disnake.Embed(title="Data Added", description=f"Added **{response}** to the database: {database}\n`Current data: {await list_data(inter, database)}`")
...
class SetupCommand(commands.Cog):
    ...
    async def setup(self, inter: disnake.GuildCommandInteraction):
        ...
        await inter.response.send_message(view=SetupCommandDropdownView(self, inter), ephemeral=True, content="Setup Menu - See <> for an example")
