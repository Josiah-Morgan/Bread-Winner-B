...
async def check_channel(table: str, inter):
  ...
  if not data: # no data, does not matter where you do the command
    return True
...
async def transaction_checks(inter, on_table: str, channel_table: str, team: disnake.Role):
  ...
  check_channel_ = await check_channel(channel_table, inter)
  if check_channel_ != True:
    return f"You can't do that command here, try doing it in: {check_channel_}"  
...
class SignCommands(commands.Cog):
  ...
  @commands.slash_command()
  async def offer(self, inter: disnake.GuildCommandInteraction, member: disnake.Member, team: disnake.Role, contract: str = None):
    """
    ...
    tr_checks = await transaction_checks(inter, "Signing", "OfferingChannel", team)
    if tr_checks != True:
  ...
  async def release(self, inter: disnake.GuildCommandInteraction, member: disnake.Member, team: disnake.Role):
    ...
    tr_checks = await transaction_checks(inter, "Signing", "SigningChannel", team)
    if tr_checks != True:
      return await inter.send(embed=error_embed("Check Failed", tr_checks), delete_after=10)
    ...
    if p_check:    
      ...
      if under_contract_[0]:
          ...
          return await member.send(embed=contract_release_embed, view=AcceptDeclineView(accept_function=accept_release, decline_function=transaction_checks, inter=inter, member=member, team=team, guild=inter.guild, inter_check=member))
    ...
    if trade_block:
      ...
      channel = inter.guild.get_channel_or_thread(int(trade_block['channel']))
      message = await channel.fetch_message(int(trade_block['message']))
      if message:
  ...
  async def demand(self, inter: disnake.GuildCommandInteraction, team: disnake.Role):
    ...
    """
    await inter.response.defer()
# contract send message to a coach
    
    tr_checks = await transaction_checks(inter, "Demand", "DemandingChannel", team)
    if tr_checks != True:
      return await inter.send(embed=error_embed("Check Failed", tr_checks), delete_after=10)
...
# send message to notifcaion channel if a coach demands or maybe to the other coahes
  ...
  async def promote(self, inter: disnake.GuildCommandInteraction, member: disnake.Member, coach_role: disnake.Role):  
    ...
    tr_checks = await transaction_checks(inter, "Signing", "SigningChannel", coach_role)
    if tr_checks != True:
      return await inter.send(embed=error_embed("Check Failed", tr_checks), delete_after=10)
  ...
  async def demote(self, inter: disnake.GuildCommandInteraction, member: disnake.Member):  
    ...
    if inter.author.id == member.id:
      return await inter.send(embed=error_embed("Can't Do That", "You can't run commands on yourself"), delete_after=10)
    check_toggle_ = await check_toggle(inter.guild.id, 'Signing')
    if not check_toggle_:
      return await inter.send(embed=error_embed("Off", "Transactions are currently off"), delete_after=10)
      
    check_channel_ = await check_channel('SigningChannel', inter)
    if check_channel_ != True:
      return await inter.send(embed=error_embed("Wrong Channel", f"You can't do that command here, try doing it in: {check_channel_}"), delete_after=10)
