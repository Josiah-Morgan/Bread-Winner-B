...
class UtilsCommands(commands.Cog):
    ...
    @commands.slash_command()
    async def members(self, inter: disnake.ApplicationCommandInteraction, role: disnake.Role):
        ...
        async for member in inter.guild.fetch_members():
            if role in member.roles:
              members_list.append(f"{member.name} ({member.mention})")
                        
            if coach_roles == "None":
              _ = "None"
            else:             
              for item in coach_roles:
                  for member in member.roles:
                    if int(role.id) == int(item):
                        coaches_list.append(f"{member.name} ({member.mention})")
      
        #print(members_list)
        print(coaches_list)
      
#if role in member.roles and coach_roles in member.roles
    ...
    @commands.slash_command()
    async def embed(self, inter: disnake.ApplicationCommandInteraction, title: str, description: str, channel: Union[disnake.TextChannel, disnake.Thread] = None):
      """
    ...
    async def helpsigning(self, inter: disnake.ApplicationCommandInteraction):
        ...
        signing_video = BotLinks.signing_video
