...
async def make_message(inter, teams, settings):
  ...
  type = settings['type']
  symbol = settings['Symbol'] if settings['Symbol'] else ''
  if type == "Schedule: Random":
    message = await create_random_schedule(design_teams, symbol) # [('Dallas', 'Giants'), ('Heat', "Balls")]
  elif type == "Schedule: Round-Robin":
    message = await create_round_robin(design_teams, symbol) # [[('Dallas', 'Giants'), ('Heat', 'Balls')], (Dallas2, Giants), (Heat2, Balls2)]
  elif type == 'Standing: Standings': #if i add more
...
async def create_random_schedule(teams, symbol):
    ...
    for match_num in range(num_matches):
        match = (teams[2 * match_num], teams[2 * match_num + 1])
...
async def create_round_robin(teams, symbol):
    ...
    return "\n\n".join([f"Round {round_num + 1}:\n" + "\n".join([f"{symbol}  {match[0]} vs. {match[1]}" for match in round_matches]) for round_num, round_matches in enumerate(schedule)])
...
class MessageDesign(disnake.ui.View):
    ...
    async def symbol_button(self, button, inter):
      await inter.send("Send your symbol", ephemeral=True)
