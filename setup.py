    ...
    for object in objects:
        if type(object) == disnake.Role:
            if await has_perms(object):
                ...
                OBJECTS_TO_NOT_ADD_REASONS += f"{object.mention} has perms, to prevent abuse, roles with permissions aren't allowed on the bot"  # , if you REALLY want this object, turn off secuity checks
        if str(object.id) in current_data:
            objects.remove(object)
            OBJECTS_TO_NOT_ADD_REASONS += (
...
async def remove_objects_database(inter, sd: dict, objects: list):
    ...
    for object in objects:
        if str(object.id) not in current_data:
            objects.remove(object)
            OBJECTS_TO_NOT_REMOVE_REASONS += (
